...
def home_user(request):
    ...
    if modo == 'recomendacoes':
        ...
        if status_filtro == 'ja-assisti':
            filmes = filmes.filter(id__in=assistidos_ids)
            
        elif status_filtro == 'meus-cadastros':
            filmes = filmes.filter(user=request.user)
            
        elif status_filtro == 'para-descobrir':
    ...
    return render(request, Area_usuario + 'home.html', {
        ...
        'page_obj': page_obj,
        'page_range': page_range, # <- NÃO ESQUEÇA DE ENVIAR ISSO PRO TEMPLATE
        'incluir_favoritos': favoritos_ids,
